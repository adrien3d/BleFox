...
{
    LIS2DS12_init(LIS2DS12_POWER_LOW, LIS2DS12_SCALE2G, drdyCB);
}
...
extern uint32_t acceleration_getElapsedTime()
{
    ...
    {
        timeElapsed = app_timer_cnt_diff_compute(app_timer_cnt_get(), timestampLastMove); // get elapsed time
    }
...
static void drdyCB(void)
{
    ...
    LIS2DS12_getALLmG(&newX, &newY, &newZ);
